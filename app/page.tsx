        ...
        <div className="overflow-y-auto flex-1 px-5 py-5 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={equipType}
                onChange={(e) => { setEquipType(e.target.value as EquipmentType); setMowerSubType(null) }}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(['riding', 'push'] as const).map((t) => (
                  <button key={t} type="button" onClick={() => setMowerSubType(t)}
                    className={`py-2 rounded-xl text-sm font-semibold border transition-colors ${
                      mowerSubType === t
                        ? 'bg-green-600 text-white border-green-600'
                        : 'bg-white text-gray-600 border-gray-200 hover:border-green-400'
                    }`}>
                    {t === 'riding' ? 'Riding' : 'Push'}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            {photoDataUrl ? (
              ...
              >
                ...
                <span className="text-xs text-gray-300">PNG or JPG · max 2 MB</span>
              </button>
            )}
...
function EquipmentPhoto({ equipment }: { equipment: Equipment }) {
  ...
  if (fallbackSrc) {
    return <img src={fallbackSrc} alt={equipment.mowerSubType!} className="w-full h-full object-cover" onError={() => setImgError(true)} />
  }
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-lg text-gray-400 hover:text-green-600 hover:bg-green-50 transition-colors"
            >
              <PencilIcon />
            </button>
          </div>
